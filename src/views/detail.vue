<template>
  <div>
    ...
    <div class="btn-container">
      <template v-if="type == 301">  
        <mt-button type="danger">求助</mt-button>
        <mt-button type="primary" @click="onSubmit">上报处理</mt-button>
      </template>
      <template v-if="type == 303">  
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        info : {
          ...
          handleStatus: null, //	处理状态:402已处理, 403:求助	number	
    ...
    mounted() {
      ...
      fetchFaultDeviceDesc({id})
        .then(res => {
          this.deviceDesc = res.body.data
          this.info.handID = res.body.data.handID
          this.info.handleStatus = res.body.data.handStatus
        })
      fetchFaultDetail({id})
        .then(res=> {
          console.log(res)
